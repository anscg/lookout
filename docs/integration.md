...
- `token` — the session credential. Give this to the user's browser.
- `sessionId` — the server-side ID. Use this in internal API calls.
...
Response includes `trackedSeconds` (tamper-proof, = distinct minute buckets × 60).
...
For the browser to upload directly to R2 via presigned URLs, your R2 bucket
needs a CORS policy. In the Cloudflare dashboard (R2 → your bucket → Settings → CORS):
...
Only `PUT` and `HEAD` are needed — the browser uploads via PUT, and the server
uses HeadObject server-side (which doesn't need CORS). Keep origins locked down
